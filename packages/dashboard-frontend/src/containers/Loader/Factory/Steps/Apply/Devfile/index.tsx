...
class StepApplyDevfile extends AbstractLoaderStep<Props, State> {
  ...
  private configureProjectRemotes(
    ...
    let gitProject = this.getGitProjectForConfigRemotes(devfile.projects);
    if (!gitProject) {
      if (!devfile.projects) {
        devfile.projects = [];
      }
  ...
  private addRemotesToProject(gitProject: V220DevfileProjectsItemsGit, checkoutRemote: GitRemote, newRemotes: GitRemote[]) {
    const gitRemotes = newRemotes.reduce((map, remote) => {
      map[remote.name] = remote.url;
      return map;
    }, {});

    if (gitProject.remotes.origin && !newRemotes.some((remote) => remote.name === 'origin')) {
      // don't remove already-existing origin remote
    ...
    gitProject.checkoutFrom = Object.assign(gitProject.checkoutFrom || {}, { remote: checkoutRemote.name });
  }

  /**
   * Returns the Git project to replace remotes for
   */
  private getGitProjectForConfigRemotes(projects: V220DevfileProjects[] | undefined) {
    if (!projects) {
      return undefined;
    }
